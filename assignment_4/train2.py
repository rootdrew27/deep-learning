...
class Augment(tf.keras.layers.Layer):
    ...
    def __init__(self, seed):
        ...
        self.randcontrast_inputs = keras.layers.RandomContrast(0.2, seed=seed)
    ...
    def call(self, inputs, labels):
        ...
        inputs = self.randcontrast_inputs(inputs)
...
for inputs, labels in train_ds.take(train_ds.cardinality().numpy()):
    all_labels.append(labels)
all_labels = tf.cast(tf.stack(all_labels, 0), dtype=tf.int32) 

counts = tf.cast(tf.math.bincount(all_labels), dtype=tf.float32)
total = tf.reduce_sum(counts)
weights = 1/counts * total/6
...
model_history = unet.fit(train_batches.map(add_sample_weights),
                        epochs=EPOCHS,
...
with open('hist_2.json', 'w') as f:
    json.dump(hist, f)



unet.save_weights("unet_2.h5") 

...
y_pred_labels = tf.argmax(y_pred, axis=1) # convert to predictions (cuz model outputs logits)
...
cm = confusion_matrix(labels_np, y_pred_labels)
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class2int.keys())
disp.plot()
plt.savefig(PLOT_PATH / 'a3q1_confusion_matrix.png')
...
plt.savefig(PLOT_PATH / 'a3q1_accuracy.png')
...
plt.ylabel('Loss')
plt.legend(loc='lower right')
plt.savefig(PLOT_PATH / 'a3q1_loss.png')